...
int main() {
    ...
    */
    ...
    for (int i = 0; i < M; i++) {
        ...
        // 여기서는 push_back을 사용한다. 어차피 이후 52행에서 매 정점마다 소트를 하기 때문에 특정 정점에서의 간선 노드들이
        // 지금 당장 어떤 순서로 추가되는지 중요하지는 않다.
        graph[u].push_back(v); // u 정점은 v 정점과 연결되는 간선을 가진다.
        graph[v].push_back(u); // v 정점은 u 정점과 연결되는 간선을 가진다.
    }

    /*
        "정점 번호가 큰 것부터 방문" = "내림차순"
        인접 리스트의 각 정점 노드(벡터)를 미리 "내림차순" 정렬해 나중에 반복문만으로 내림차순 방문 가능하도록 하자.
    */
    //“BFS 도중에 정렬하면 안 되나?” => 시간 복잡도 폭증돼서 먼저 해야함.
    for (int i = 1; i <= N; i++) {
        sort(graph[i].begin(), graph[i].end(), greater<int>());
    }

    // order[i]: visited 배열 + 방문 순서 배열을 겸한다
    // 정점 i가 몇 번째로 방문되었는지 저장
    // 정점 0은 없음 + 방문 안한 경우를 처리하기 위해 사용하려 함.
    // N+1로 크기 지정. 0으로 초기화.
    // order 인덱싱에는 정점번호를 쓰고 그 칸에는 방문 순서 번호가 들어간다. **
    ...
    // 시작 정점 R을 첫 번째로 방문
    // postfix라서 시작 정점 R에는 1이 들어가고 이후 증가된다.
    ...
    while (!q.empty()) { // q에 정점이 남아있으면
        ...
        for (int next : graph[cur]) { // 현재 정점과 이어지는 다음 정점 = next, 처음에는 R의 다음 정점을 오름차순으로 순회할 것이다.
            ...
            if (order[next] == 0) { // 다음 정점 방문한 적 없으면
                order[next] = cnt++; // 방문 순서 기록이 곧 방문 처리(!= 0)도 하는 것이다.
                q.push(next); // 다음 레벨에서 탐색하도록 큐에 추가
                // order의 인덱싱, q 내부에는 정점번호가 사용된다.
                // 방문 순서및여부 기록은 push보다 먼저해야 한다. push 먼저 하고 방문기록하면 조건문에서 
            }
        }
    }
